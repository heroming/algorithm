...
class Solution :
    ...
    def backPackII(self, m, w, v) :
        if not w or not v : return 0
        n, s = len(w), sum(w)
        if s <= m : return sum([x for x in v if x > 0])
        dp = [0 for i in xrange(m + 1)]
        for k in xrange(n) :
            idx = m
            while idx >= w[k] :
                dp[idx] = max(dp[idx], dp[idx - w[k]] + v[k])
                idx -= 1
        return dp[m]
...
if __name__ == "__main__" :
    ...
    so = Solution()
    print so.backPackII(10, [2, 3, 5, 7], [1, 5, 2, 4])
