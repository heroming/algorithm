...
class Solution :
    ...
    def query(self, v, l, r) :
        ...
        def dfs(v, s, e) :
            if s <= v.start and v.end <= e :
                return v.count
            m = (v.start + v.end) >> 1
            if e <= m :
                return dfs(v.left, s, e)
            if m < s :
                return dfs(v.right, s, e)
            return dfs(v.left, s, e) + dfs(v.right, s, e)
        
        if v is None : return 0
        a = max(l, v.start)
        b = min(r, v.end)
        return 0 if b < a else dfs(v, a, b)
...
if __name__ == "__main__" :
    ...
    so = Solution()
    print so.maxProduct([-3, 2])
    print so.maxProduct([2, 3, -2, 0])
