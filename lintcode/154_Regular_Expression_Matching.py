...
class Solution :
    ...
    def isMatch(self, s, t) :
        ...
        def dfs(s, t) :
            if len(t) == 0 : return len(s) == 0
            if len(s) == 0 : return valid(t)
            if t[0] == '.' : 
                if len(t) >= 2 and t[1] == '*' :
                ...
                return dfs(s[1:], t[1:])
            if len(t) >= 2 and t[1] == '*' :
                if dfs(s, t[2:]) : return True
                k = 0
                while k < len(s) :
                    if s[k] != t[0] : break
                    if dfs(s[k + 1:], t[2:]) : return True
                    k += 1
                return False
            return s[0] == t[0] and dfs(s[1:], t[1:])

        # dfs method failed : time limit exceeded
        #return dfs(s, t)
        
        n, m = len(s), len(t)
        if m == 0 : return n == 0
        if n == 0 : return valid(t)
...
if __name__ == "__main__" :
    ...
    print so.isMatch("aaaaaaaaaaaaab", "a*a*a*a*a*a*a*a*a*a*c*.")
