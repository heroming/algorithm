...
class Solution :
    ...
    def maxNumber(self, a, b, h) :
        
        def solve(v, t) :
            n, ret = len(v), []
            for k in xrange(n) :
                while ret and t < len(ret) + n - k and ret[-1] < v[k] : ret.pop()
                if len(ret) < t : ret.append(v[k])
            return ret
 
        def merge(u, v) :
            return [max(u, v).pop(0) for _ in u + v]
 
        ans = []
        n, m = len(a), len(b)
        for x in xrange(max(0, h - m), min(h, n) + 1) :
             ans = max(ans, merge(solve(a, x), solve(b, h - x)))
        return ans
    
 
if __name__ == "__main__" :
    so = Solution()
    print so.maxNumber([3, 4, 6, 5], [9, 1, 2, 5, 8, 3], 5)
