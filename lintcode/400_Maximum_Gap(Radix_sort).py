...
class Solution :
    ...
    def maximumGap(self, v) :
 
        def count_sort(shift, u) :
            bit = (1 << N) - 1
            count = [0 for i in xrange(1 << N)]
            for x in v :
                count[(x >> shift) & bit] += 1
...
if __name__ == "__main__" :
    so = Solution()
    import random
    v = [random.randint(1, 100000000) for i in xrange(1000000)]
    #print so.maximumGap([1, 9, 2, 5])
    print so.maximumGap(v)
