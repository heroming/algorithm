        ...
        def dfs(k, pre, suf) :
            if k == 0 :
                if not suf :
                    ans.append(pre)
                return
            if not suf or 3 * k < len(suf): return
            if k != 4 : pre += '.'
            dfs(k - 1, pre + suf[:1], suf[1:])
            if 2 <= len(suf) and '10' <= suf[:2] <= '99' :
                dfs(k - 1, pre + suf[:2], suf[2:])
            if 3 <= len(suf) and '100' <= suf[:3] <= '255' :
                dfs(k - 1, pre + suf[:3], suf[3:])
