...
class Solution :
    ...
    def longestConsecutive(self, num) :
        ...
        def brute_force(num) :
            ...
            num.sort()
            ans, cnt = 1, 1
            for k in xrange(1, len(num)) :
                if num[k - 1] == num[k] : continue
                elif num[k - 1] + 1 == num[k] :
                    cnt += 1
                    ans = max(ans, cnt)
                else :
                    cnt = 1
        ...
        def find_union_set(v) :
            ...
            for k in xrange(n) :
                if v[k] not in dic :
                    dic[v[k]] = m
                    m += 1
            count = [1 for i in xrange(m)]
            father = [i for i in xrange(m)]
        ...
        return find_union_set(num)
 
 
if __name__ == "__main__" :
    so = Solution()
    print so.longestConsecutive([0,0,-1])
    print so.longestConsecutive([1,3,4,2,3,3,3,7,8,1])
