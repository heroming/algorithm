...
class Solution :
    ...
    def longestPalindrome(self, s) :
        ...
        def manachar(s) :
            ...
            for k in xrange(1, n) :
                if k < mx :
                    p[k] = min(p[(id << 1) - k], mx - k)
                while k - p[k] >= 1 and k + p[k] + 1 < n and v[k - p[k] - 1] == v[k + p[k] + 1] :
                    p[k] += 1
                if mx < k + p[k] :
                    id = k
                    mx = k + p[k]
...
if __name__ == "__main__" :
    ...
    print so.longestPalindrome("babcdb")
    print so.longestPalindrome("abczdcab")
    print so.longestPalindrome("zdcab")
    print so.longestPalindrome("zfofoofdcab")
    print so.longestPalindrome("abczcafkdjhafoafijfajifaiab")
