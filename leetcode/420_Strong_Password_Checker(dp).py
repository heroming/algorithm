...
class Solution(object) :
    def strongPasswordChecker(self, s) :
        ...
        def update(dic, i, j, k, c, w) :
            if c in dic[i][j][k] :
                dic[i][j][k][c] = min(dic[i][j][k][c], w)
            else :
                dic[i][j][k][c] = w
        ...
        if n <= MAX :
            ...
            while i < n :
                if s[i - 2] == s[i - 1] == s[i] :
                    ...
                    rep += 1
                i += 1
            return max(MIN - n, rep, need)
 
        # dp[21][8][2] = {c:x}: [lenght of password][bit for upper, lower and digit][repeat count], c is last charector, x is minimal edit
        dp = [[[{} for _ in xrange(2)] for _ in xrange(8)] for _ in xrange(21)]
        dp[0][0][0]['**'] = 0

        for it in xrange(n) :
            c, m = s[it], min(it + 1, MAX + 1)
            mp = [[[{} for _ in xrange(2)] for _ in xrange(8)] for _ in xrange(21)]
            for i in xrange(m) :
                for j in xrange(8) :
                    for k in xrange(2) :
                        for x in dp[i][j][k] :
                            w = dp[i][j][k][x]
                            # delete c
                            update(mp, i, j, k, x, w + 1)
                            if i < MAX :
                                # change c
                                update(mp, i + 1, j | 1, 0, '**', w + 1)
                                update(mp, i + 1, j | 2, 0, '**', w + 1)
                                update(mp, i + 1, j | 4, 0, '**', w + 1)
                                # add c
                                if x == '**' or c != x:
                                    if 'A' <= c <= 'Z' : update(mp, i + 1, j | 1, 0, c, w)
                                    elif 'a' <= c <= 'z' : update(mp, i + 1, j | 2, 0, c, w)
                                    elif '0' <= c <= '9' : update(mp, i + 1, j | 4, 0, c, w)
                                    else : update(mp, i + 1, j, 0, c, w)
                                elif c == x :
                                    if k == 0 : update(mp, i + 1, j, 1, c, w)
            dp = mp
        ans = n
        for i in xrange(MIN, MAX + 1) :
            for k in xrange(2) :
                for x in dp[i][7][k] :
                    ans = min(ans, dp[i][7][k][x])
        return ans

if __name__ == "__main__" :
    so = Solution()
    print so.strongPasswordChecker("dddAd")
    print so.strongPasswordChecker("######!!!!!****&&&&%&%")
