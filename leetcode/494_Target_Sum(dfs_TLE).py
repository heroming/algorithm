...
class Solution(object) :
    def findTargetSumWays(self, u, w) :
        def dfs(k, x) :
            if k == n : return 0
            if x + re[k] == w or x - re[k] == w : return 1
            if x + re[k] < w or w < x - re[k] : return 0
            return dfs(k + 1, x + v[k]) + dfs(k + 1, x - v[k])
...
if __name__ == "__main__" :
    so = Solution()
    print so.findTargetSumWays([1,2,3,4], 0)
    print so.findTargetSumWays([3,6,1,8,3,5,9,7,9,7,3,6,3,1,8,4,3,8,1,2],-57)
    print so.findTargetSumWays([25,33,27,23,46,16,10,27,33,2,12,2,29,44,49,40,32,46,7,50],4)
    print so.findTargetSumWays([34,21,12,36,16,7,31,7,41,49,7,48,22,19,32,46,19,18,44,34],47)
