...
class Tree(object) :
    ...
    def __init__(self, n) :
        self.n = n
        self.data = [0 for i in xrange(n << 2)]

    def update(self, v, l, r, s, e) :
        if self.data[v] == 1 : return
        if s <= l and r <= e :
            self.data[v] = 1
            return
        m = (l + r) >> 1
        if e <= m :
            self.update(v << 1, l, m, s, e)
        elif m < s :
            self.update(v << 1 | 1, m + 1, r, s, e)
        else :
            self.update(v << 1, l, m, s, e)
            self.update(v << 1 | 1, m + 1, r, s, e)

    def query(self, v, l, r, ans) :
        if self.data[v] == 1 : return
        if l == r :
            ans.append((l, r))
            return
        m = (l + r) >> 1
        self.query(v << 1, l, m, ans)
        self.query(v << 1 | 1, m + 1, r, ans)
...
class Solution(object) :
    ...
    def employeeFreeTime(self, sc) :
        data = []
        for s in sc :
            for t in s :
                data.append(t.start)
                data.append(t.end)
        self.data = sorted(set(data))
        n = len(self.data)
        ...
        ans = []
        tree.query(1, 1, n, ans)
        ans.pop()
        ret = []
        for (a, b) in ans :
            ret.append(self.getInterval(a, b))
        return ret
...
if __name__ == "__main__" :
    ...
    b = [Interval(1, 3)]
    c = [Interval(4, 10)]
    ret = so.employeeFreeTime([a, b, c])
    print [(x.start, x.end) for x in ret]

    a = [Interval(1, 3), Interval(6, 7)]
    b = [Interval(2, 4)]
    c = [Interval(2, 5), Interval(9, 12)]
    ret = so.employeeFreeTime([a, b, c])
    print [(x.start, x.end) for x in ret]
