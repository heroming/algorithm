...
class Solution(object) :
    def characterReplacement(self, s, m) :
        if not s : return 0
        ...
        v = [[] for i in xrange(letter)]
        for i in xrange(n) :
            k = ord(s[i]) - ord_a
            v[k].append(i)
            
        ans = 0
        for k in xrange(letter) :
            ret = 0
            u = v[k]
            t = len(u)
            ...
            while r < t :
                while m < (u[r] - u[l]) - (r - l) :
                    l += 1
                ret = max(ret, u[r] - u[l] + 1 + m - (u[r] - u[l]) + (r - l))
                r += 1
            ans = max(ans, ret)
        return ans
...
if __name__ == "__main__" :
    so = Solution()
    print so.characterReplacement("ABAB", 2)
    print so.characterReplacement("AABABBA", 1)
