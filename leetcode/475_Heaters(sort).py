...
class Solution(object) :
    def findRadius(self, houses, heaters) :
        ...
        if not houses or not heaters : return 0
        houses.sort()
        heaters.sort()
        ...
        ans, n = 0, len(heaters)
        for x in houses :
            while idx + 1 < n and heaters[idx + 1] <= x : idx += 1
            if idx + 1 < n :
                ans = max(ans, min(abs(x - heaters[idx]), heaters[idx + 1] - x))
            else :
                ans = max(ans, abs(heaters[idx] - x))
            print idx, x, ans
        return ans
...
if __name__ == "__main__" :
    so = Solution()
    #print so.findRadius([1, 2, 3, 5, 15], [2, 30])
    #print so.findRadius([1, 2, 3], [2])
    #print so.findRadius([1, 2, 3, 4], [1, 4])
    print so.findRadius([1, 2, 3, 8], [1, 4])
    print so.findRadius([617819336,399125485,156091745,356425228], [585640194,937186357])
