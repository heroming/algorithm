...
class MyTrieNode(object) :
    ...
    def __init__(self) :
        self.word = False
        self.data = [None for i in xrange(MyTrieNode.N)]
...
class MyTrie(object) :
    ...
    def insert(self, s) :
        ...
        for k in xrange(len(s)) :
            idx = ord(s[k]) - MyTrieNode.X
            if node.data[idx] is None :
                node.data[idx] = MyTrieNode()
            node = node.data[idx]
        node.word = True

    def search(self, k, s, dp, stack) :
        node = self.root
        for i in xrange(k, len(s)) :
            idx = ord(s[i]) - MyTrieNode.X
            if node.data[idx] is None : return
            node = node.data[idx]
            if node.word :
                if dp[i + 1] == -1 and i + 1 < len(s) :
                    stack.append(i + 1)
                dp[i + 1] = dp[k] + 1
...
class Solution(object) :
    def findAllConcatenatedWordsInADict(self, words) :
        ...
        def concatenate(s) :
            ...
            while stack :
                k = stack.pop()
                trie.search(k, s, dp, stack)
                if dp[n] > 1 : return True 
            return False
 
        ans = []
        trie = MyTrie()
        for s in words : trie.insert(s)
        for s in words :
            if concatenate(s) :
                ans.append(s)
        return ans
...
if __name__ == "__main__" :
    so = Solution()
    print so.findAllConcatenatedWordsInADict(["b","bb","bbb", "bbbb", "bbbbb", "bbbbbb", "bbbbbbb"])
    #print so.findAllConcatenatedWordsInADict(["a","c","ac"])
