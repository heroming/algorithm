...
class Solution(object) :
    def makesquare(self, v) :
        
        def dfs(k, w, u, v, s) :
            if w == s : return True
            if k == len(v) : return False
            if w + v[k] <= s :
                u.append(v[k])
                if dfs(k + 1, w + v[k], u, v, s) : return True
                u.pop()
            if dfs(k + 1, w, u, v, s) : return True
            return False
        
        def solve(v, s) :
            h, u = v[:], []
            if not dfs(0, 0, u, v, s) : return False
            del v[:]
            k, m = 0, len(u)
            for x in h :
                if k < m and x == u[k] :
        ...
        n, s = len(v), sum(v)
        if n < 4 or s % 4 : return False
        s >>= 2
        v.sort(lambda a, b: b - a)
        for i in xrange(3) :
            if not solve(v, s) : return False
        return True
...
if __name__ == "__main__" :
    ...
    print so.makesquare([6,2,2,4,3,3,5,1,4,5,5])
