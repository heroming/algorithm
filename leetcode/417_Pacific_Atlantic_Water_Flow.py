...
class Solution(object) :
    def pacificAtlantic(self, mat) :
        ...
        inq = [[False for i in xrange(m)] for j in xrange(n)]
        que = []
        for i in xrange(n) :
            dis[i][0] |= 1
            inq[i][0] = True
        ...
        while que :
            ...
            for i in xrange(len(dx)) :
                a = x + dx[i]
                b = y + dy[i]
                if a >= 0 and a < n and b >= 0 and b < m and mat[x][y] <= mat[a][b] and (dis[x][y] | dis[a][b]) != dis[a][b] :
                    dis[a][b] |= dis[x][y]
                    if not inq[a][b] :
                        inq[a][b] = True
                        que.append((a, b))
...
if __name__ == "__main__" :
    so = Solution()
    print so.pacificAtlantic([[3,3,3,3,3,3],[3,0,3,3,0,3],[3,3,3,3,3,3]])
    #print so.pacificAtlantic([[1,2,2,3,5],[3,2,3,4,4],[2,4,5,3,1],[6,7,1,4,5],[5,1,1,2,4]])
