...
class AllOne(object) :
    ...
    def inc(self, key) :
        if key in self.dic :
            self.dic[key] += 1
            x = self.dic[key]
            self.data[x - 1].remove(key)

            if x < len(self.data) :
                self.data[x].add(key)
            else :
                self.data.append(set([key]))
 
            if self.min_n + 1 == x and len(self.data[x - 1]) == 0 :
                self.min_n = x
    ...
    def dec(self, key):
        if key in self.dic :
            if self.dic[key] == 1 :
                self.n -= 1
                del self.dic[key]
                self.data[1].remove(key)
                
                if self.n == 0 :
                    self.min_n = self.max_n = 0
                elif len(self.data[1]) == 0 :
                    for i in xrange(2, self.max_n + 1) :
                        if self.data[i] :
                            self.min_n = i
                            break
            else :
                ...
                self.data[x - 1].add(key)
                self.min_n = min(self.min_n, x - 1)
                if len(self.data[self.max_n]) == 0 :
                    self.max_n -= 1

    def getMaxKey(self):
        for x in self.data[self.max_n] : return x

    def getMinKey(self):
        for x in self.data[self.min_n] : return x
