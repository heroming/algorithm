...
class Solution(object) :
    def validIPAddress(self, ip) :
        ...
        def invalid4(x) :
            try :
                w = int(x)
                if x[0] == '0' and w > 0 : return True
                if w == 0 and len(x) > 1 : return True
                if 0 <= w <= 255 : return False
                return True
            except :
                return True
        ...
        def invalid8(x) :
            if 1 <= len(x) <= 4 :
                for c in x :
                    if not ('0' <= c <= '9' or 'a' <= c <= 'f') : return True
                return False
            return True
        ...
        def validIPv4(u) :
            ...
            for x in u :
                if invalid4(x) : return False
            return True
        
        def validIPv6(u) :
            if len(u) != 8 : return False
            for x in u :
                if invalid8(x) : return False
            return True

        ip = ip.lower()
        if '.' in ip :
            u = ip.split('.')
            if validIPv4(u) : return "IPv4"
        else :
            u = ip.split(':')
            if validIPv6(u) : return "IPv6"
        return "Neither"
