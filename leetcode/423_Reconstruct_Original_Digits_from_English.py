...
class Solution(object) :
    def originalDigits(self, s) :
        ...
        finish = False
        ret = [0 for i in xrange(len(digit))]
        ...
        while not finish :
            ...
            if not finish :
                ret[x] += w
                for c in digit[x] :
                    if x in mp[c] : mp[c].remove(x)
                for c in digit[x] :
                    dic[c] -= w

        ans = []
        for i in xrange(len(ret)) :
            if ret[i] : ans.append(str(i) * ret[i])
        return ''.join(ans)
...
if __name__ == "__main__" :
    ...
    print so.originalDigits("fviefuro")
    print so.originalDigits("ereht")
    print so.originalDigits("esnve")
