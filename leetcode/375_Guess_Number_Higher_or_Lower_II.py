...
class Solution(object) :
    def getMoneyAmount(self, n) :
        ...
        if n == 3 : return 2
        N = n + 1
        inf = 0x7FFF7FFF7FFF7FFFL
        dp = [[inf for _ in xrange(N)] for _ in xrange(N)]
        #for i in xrange(N) :
            #for j in xrange(i) :
                #dp[i][j] = 0
        for i in xrange(1, N) : dp[i][i] = 0
        for i in xrange(1, N - 1) : dp[i][i + 1] = i
        for i in xrange(1, N - 2) : dp[i][i + 2] = i + 1
        for k in xrange(3, N) :
            for i in xrange(1, N - k) :
                for x in xrange(i + 1, i + k) :
                    dp[i][i + k] = min(dp[i][i + k], max(dp[i][x - 1], dp[x + 1][i + k]) + x)
        return dp[1][n]
