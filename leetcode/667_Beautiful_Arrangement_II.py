...
class Solution(object) :
    ...
    def constructArray(self, n, k) :
        rest = [x + 1 for x in xrange(k + 1, n)]
        build = [(k + 2) >> 1]

        sign, val, idx = 1, 1, 0
        while len(build) < k + 1 :
            build.append(build[-1] + val * sign)
            sign, val = sign * -1, val + 1
        # print build, rest
        if k & 1 : return build + rest
        return build[:k] + rest + build[k:]
...
if __name__ == "__main__" :
    ...
    print so.constructArray(6, 4)
    for i in xrange(2000) :
        n = randint(3, 10000)
        k = randint(1, n - 1)
        v = so.constructArray(n, k)
        #kk = so.check(v)
        #if k != kk : print n, k
