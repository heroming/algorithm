...
class User(object) :
    ...
    def post(self, tid, time) :
        self.postList.append((time, tid))
        
    def follow(self, tid) :
        if tid not in self.followList :
            self.followList[tid] = True
    
    def unfollow(self, tid) :
        if tid in self.followList :
            del self.followList[tid]
    ...
    def getPost(self) :
        ...
        if n <= Twitter.SHOW :
            ...
            return self.postList[n - Twitter.SHOW:]
...
class Twitter(object) :
    ...
    def postTweet(self, userId, tweetId) :
        if userId not in self.user :
            ...
            self.user[userId].follow(userId)
        self.time += 1
        self.user[userId].post(tweetId, self.time)

    def getNewsFeed(self, userId) :
        if userId not in self.user : return []
        heap = MinHeap()
        for followId in self.user[userId].getFollow() :
            for tweet in self.user[followId].getPost() :
                if heap.size() < Twitter.SHOW :
                    heap.push(tweet)
                else :
                    if heap.top()[0] < tweet[0] :
                        heap.pop();
                        heap.push(tweet)
        ans = []
    ...
    def follow(self, followerId, followeeId) :
        if followerId not in self.user :
            ...
            self.user[followerId].follow(followerId)
        if followeeId not in self.user :
            self.user[followeeId] = User()
            self.user[followeeId].follow(followeeId)
