...
class Solution(object) :
    def find132pattern(self, v) :
        ...
        for i in xrange(1, n) :
            min_x = min(min_x, v[i])
            bit.update(v[i], -1)
            if min_x + 1 < v[i] and bit.query(v[i] - 1) - bit.query(min_x) > 0 :
                return True
        return False

if __name__ == "__main__" :
    so = Solution()
    print so.find132pattern([1, 0, 1, -4, -3])
    #print so.find132pattern([1, 3, 2, 0])
    #print so.find132pattern([0, 1, 2, 3, 0])
    #print so.find132pattern([1, 3, 2, 2, 2, 2, 3, 3, 3, 0])
